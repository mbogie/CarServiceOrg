trigger OverlappingError on Contract__c (before insert, before update) {
    ...
    existingContracts = [SELECT Name, Mechanic__c, Workshop__c, Start_Date__c, End_Date__c FROM Contract__c
            ...
            for (Contract__c agreementsContract : similarContracts) {
                if ((triggerContract.Start_Date__c != null) && (triggerContract.End_Date__c != null) && (agreementsContract.End_Date__c != null)) {
                    if (triggerContract.Start_Date__c <= agreementsContract.End_Date__c && triggerContract.End_Date__c >= agreementsContract.Start_Date__c) {
                        errorMessage += String.format(System.Label.Error_Overlapping_From_To, new List<String>{agreementsContract.Name,agreementsContract.Start_Date__c.format(),agreementsContract.End_Date__c.format()});
                        error = true;
                    }
                }
                ...
                if ((triggerContract.End_Date__c == null) && (agreementsContract.End_Date__c == null)) {
                    errorMessage += String.format(System.Label.Error_Overlapping_From, new List<String>{agreementsContract.Name,agreementsContract.Start_Date__c.format()});
                    error = true;
                }
            }
            if (error) {
